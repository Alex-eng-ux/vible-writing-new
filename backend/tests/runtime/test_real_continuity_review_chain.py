"""Task 9 真实 Continuity/Review Agent 链路测试（默认 HTTP mock，不访问网络）。

覆盖：
- 成功链路：创建带已接受基线文本的场景运行 -> 经真实 Provider（MockTransport）
  WritingAgent/ContinuityAgent/ReviewAgent 依次返回结构化结果 -> 运行进入暂停
  终态（revision 需澄清），不创建任何版本/候选/Canon 数据；
- 失败链路：Continuity/Review Provider 401 / 超时 / 非法结构化输出 -> 运行
  failed + 对应 LLM_* 错误码，不创建任何版本/候选/Canon 数据；
...
def _create_scene_run_with_accepted_baseline(db, run_id: str) -> None:
    """创建带已接受基线文本的 queued 场景运行。

    设置 scene_brief（保证 WritingAgent 调模型）与一个已接受版本（保证
    Continuity/ReviewAgent 拿到 accepted_text 后调模型）。
    """
...
def test_real_chain_success_runs_all_agents(db) -> None:
    """成功链路：Writing -> Continuity -> Review 均经 provider 返回结构化结果，无版本写入。"""
    run_id = "g-cr-1"
    _create_scene_run_with_accepted_baseline(db, run_id)
    before = _counts(db)
    seen_nodes: list[str] = []
    ...
    # Revision 缺 base_scene_revision_id -> needs_clarification -> 暂停终态。
    assert row.status in ("pending_clarification", "waiting_feedback")
    assert _events(db, run_id)[0] == "run_queued"
    # 三个场景 Agent 均被真实 Provider（mock）调用一次。
    assert seen_nodes == ["writing", "continuity", "review"]
    _assert_no_writes(db, before)
    # 观测自动埋点：continuity / review 挂载 llm 节点。
    assert wiring.local is not None
    node_names = [str(r.get("node_name")) for r in wiring.local.records]
    assert any(n.startswith("continuity:llm:") for n in node_names)
    assert any(n.startswith("review:llm:") for n in node_names)


def test_real_chain_continuity_401_fails_without_writes(db) -> None:
    """失败链路：Continuity Provider 401 -> 运行 failed + LLM_AUTH_ERROR，无写入。"""
    run_id = "g-cr-2"
    _create_scene_run_with_accepted_baseline(db, run_id)
    before = _counts(db)
...
def test_real_chain_review_timeout_fails_without_writes(db) -> None:
    """失败链路：Review Provider 超时 -> 运行 failed + LLM_UNAVAILABLE，无写入。"""
    run_id = "g-cr-3"
    _create_scene_run_with_accepted_baseline(db, run_id)
    before = _counts(db)
...
def test_real_chain_continuity_invalid_output_fails(db) -> None:
    """失败链路：Continuity 非法结构化输出 -> 运行 failed + LLM_RESPONSE_INVALID。"""
    run_id = "g-cr-4"
    _create_scene_run_with_accepted_baseline(db, run_id)
    before = _counts(db)
...
def test_real_chain_with_real_deepseek(db) -> None:
    """真实模型 smoke：用真实 DeepSeek 走 Writing/Continuity/Review 完整链路（门控）。"""
    run_id = "g-cr-live"
    _create_scene_run_with_accepted_baseline(db, run_id)
    before = _counts(db)
    factory = sessionmaker(bind=db.bind, expire_on_commit=False)
